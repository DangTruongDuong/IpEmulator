...
pub fn app_picker(
    ...
    let apps_dir = {
        let config_path = paths::user_data_base_path().join("IpEmulator_apps_path.txt");
        if config_path.is_file() {
            if let Ok(content) = std::fs::read_to_string(&config_path) {
                ...
                if !trimmed.is_empty() {
                    ...
                    paths::user_data_base_path().join(paths::APPS_DIR)
                }
            } else {
                paths::user_data_base_path().join(paths::APPS_DIR)
            }
    ...
    let apps: Result<Vec<AppInfo>, String> = if !apps_dir.is_dir() {
        if apps_dir != paths::user_data_base_path().join(paths::APPS_DIR) {
            Err(format!("The custom directory {} couldn't be found. Please select a valid directory.", apps_dir.display()))
        } else {
...
- (())selectAppsFolder {
    env.objc.borrow_mut::<AppPickerDelegateHostObject>(this).select_folder_tapped = true;
}
- (())scaleHackDefault {
...
fn show_app_picker_gui(
    ...
    let bg_color = msg_class![env; UIColor colorWithRed:(0.09 as CGFloat) green:(0.09 as CGFloat) blue:(0.11 as CGFloat) alpha:(1.0 as CGFloat)];
    ...
    // Version label
    {
        ...
        let text = ns_string::from_rust_string(
            env,
            format!(
                "IpEmulator v{} — Copyright by DuOzeNg",
                crate::VERSION
            ),
        );
    ...
    let header_frame = CGRect {
        origin: CGPoint { x: 10.0, y: 15.0 },
        size: CGSize {
            width: app_frame.size.width - 20.0,
            height: 25.0,
        },
    };
    let header_label: id = msg_class![env; UILabel alloc];
    let header_label: id = msg![env; header_label initWithFrame:header_frame];
    let title_text = ns_string::from_rust_string(env, "IpEmulator Library".to_string());
    () = msg![env; header_label setText:title_text];
    () = msg![env; header_label setTextAlignment:UITextAlignmentCenter];
    let title_font: id = msg_class![env; UIFont boldSystemFontOfSize:(20.0 as CGFloat)];
    () = msg![env; header_label setFont:title_font];
    let white_color: id = msg_class![env; UIColor whiteColor];
    () = msg![env; header_label setTextColor:white_color];
    let clear_color: id = msg_class![env; UIColor clearColor];
    () = msg![env; header_label setBackgroundColor:clear_color];
    () = msg![env; main_view addSubview:header_label];

    // Subtitle / Copyright
    let sub_frame = CGRect {
        origin: CGPoint { x: 10.0, y: 38.0 },
        size: CGSize {
            width: app_frame.size.width - 20.0,
            height: 15.0,
        },
    };
    let sub_label: id = msg_class![env; UILabel alloc];
    let sub_label: id = msg![env; sub_label initWithFrame:sub_frame];
    let sub_text = if super::branding().is_empty() {
        ns_string::from_rust_string(env, "— Copyright by DuOzeNg —".to_string())
    } else {
        ns_string::from_rust_string(env, format!("— {} BUILD by DuOzeNg —", super::branding()))
    };
    () = msg![env; sub_label setText:sub_text];
    () = msg![env; sub_label setTextAlignment:UITextAlignmentCenter];
    let sub_font: id = msg_class![env; UIFont boldSystemFontOfSize:(11.0 as CGFloat)];
    () = msg![env; sub_label setFont:sub_font];
    let sub_color = if super::branding() == "UNOFFICIAL" {
        msg_class![env; UIColor redColor]
    } else if super::branding() == "PREVIEW" {
        msg_class![env; UIColor magentaColor]
    } else {
        msg_class![env; UIColor lightGrayColor]
    };
    () = msg![env; sub_label setTextColor:sub_color];
    () = msg![env; sub_label setBackgroundColor:clear_color];
    () = msg![env; main_view addSubview:sub_label];
    ...
    make_button_row(
        ...
        buttons_row2_center,
        &[
            ("Select Folder", "selectAppsFolder"),
            ("Copyright", "copyrightInfoShow"),
    ...
    fn update_quick_option_buttons(env: &mut Environment, buttons: &[id], selected_idx: usize) {
        for (idx, &button) in buttons.iter().enumerate() {
            let color: id = if idx == selected_idx {
                msg_class![env; UIColor colorWithRed:(0.35 as CGFloat) green:(0.34 as CGFloat) blue:(0.84 as CGFloat) alpha:(1.0 as CGFloat)] // Indigo
            } else {
                msg_class![env; UIColor colorWithRed:(0.16 as CGFloat) green:(0.18 as CGFloat) blue:(0.22 as CGFloat) alpha:(1.0 as CGFloat)] // Charcoal
            };
    ...
    let app_path = loop {
        ...
        let select_folder_tapped = std::mem::take(&mut host_obj.select_folder_tapped);
        if select_folder_tapped {
            ...
            #[cfg(target_os = "windows")]
            {
                ...
                $dialog.Description = 'Select IpEmulator Games Folder';
                ...
                if let Ok(out) = output {
                    if out.status.success() {
                        ...
                        if !path_str.is_empty() {
                            let config_path = paths::user_data_base_path().join("IpEmulator_apps_path.txt");
                            if std::fs::write(&config_path, &path_str).is_ok() {
                                path_changed = true;
                            }
                        }
                    }
            ...
            #[cfg(target_os = "macos")]
            {
                let applescript = "POSIX path of (choose folder with prompt \"Select IpEmulator Games Folder\")";
            ...
            #[cfg(target_os = "linux")]
            {
                let output = std::process::Command::new("zenity")
                    .arg("--file-selection")
                    .arg("--directory")
                    .arg("--title=Select IpEmulator Games Folder")
                    .output()
                    .or_else(|_| {
            ...
            {
                crate::window::show_error_messagebox(None, "Android users should use the Documents Provider to copy games into the IpEmulator directory.");
            }
            ...
            if path_changed {
                let apps_dir = {
                    let config_path = paths::user_data_base_path().join("IpEmulator_apps_path.txt");
                    if let Ok(content) = std::fs::read_to_string(&config_path) {
                        PathBuf::from(content.trim().to_string())
                    } else {
                        paths::user_data_base_path().join(paths::APPS_DIR)
                    }
                };
                apps = enumerate_apps(&apps_dir);
                if let Some(ref mut grid_stuff) = icon_grid_stuff {
                    match &mut apps {
                        Ok(ref mut new_apps) => {
                            let mut start = 0;
                            let total_app_count = new_apps.len();
                            let mut pages = Vec::new();
                            while start < total_app_count {
                                let mut end = start + grid_stuff.icon_buttons_and_labels.len();
                                if start > 0 {
                                    end -= 1;
                                }
                            ...
                            grid_stuff.pages = pages;
                            update_icon_grid(env, grid_stuff, new_apps, 0);
                        }
                        Err(e) => {
                            crate::window::show_error_messagebox(None, e);
                        }
                    }
                }
            }
...
fn make_button_row(
    ...
    for (title_text, selector) in buttons {
        ...
        let btn_bg = msg_class![env; UIColor colorWithRed:(0.16 as CGFloat) green:(0.18 as CGFloat) blue:(0.22 as CGFloat) alpha:(1.0 as CGFloat)];
        () = msg![env; button setBackgroundColor:btn_bg];
        let btn_text_color = msg_class![env; UIColor whiteColor];
        () = msg![env; button setTitleColor:btn_text_color forState:UIControlStateNormal];
        ...
        if let Some(font_size) = font_size {
            ...
            let font: id = msg_class![env; UIFont boldSystemFontOfSize:(12.0 as CGFloat)];
            () = msg![env; label setFont:font];
        }
...
fn setup_copyright_info(
    ...
    let bg_color = msg_class![env; UIColor colorWithRed:(0.11 as CGFloat) green:(0.12 as CGFloat) blue:(0.15 as CGFloat) alpha:(1.0 as CGFloat)];
    () = msg![env; main_view setBackgroundColor:bg_color];
    ...
    let clear_color = msg_class![env; UIColor clearColor];
...
fn setup_quick_options(
    ...
    // Close button
    {
        ...
        let button: id = msg_class![env; UIButton buttonWithType:UIButtonTypeRoundedRect];
        let btn_bg = msg_class![env; UIColor colorWithRed:(0.20 as CGFloat) green:(0.22 as CGFloat) blue:(0.27 as CGFloat) alpha:(1.0 as CGFloat)];
        () = msg![env; button setBackgroundColor:btn_bg];
        let btn_text_color = msg_class![env; UIColor whiteColor];
        () = msg![env; button setTitleColor:btn_text_color forState:UIControlStateNormal];
        ...
        let label: id = msg![env; button titleLabel];
        let font: id = msg_class![env; UIFont boldSystemFontOfSize:(24.0 as CGFloat)];
        () = msg![env; label setFont:font];
    ...
    for (i, row) in rows.iter().enumerate() {
        ...
        match *row {
            RowKind::Label(text) => {
                ...
                () = msg![env; label setText:text];
                () = msg![env; label setTextAlignment:UITextAlignmentCenter];
                let label_color = msg_class![env; UIColor whiteColor];
                () = msg![env; label setTextColor:label_color];
                let clear_color = msg_class![env; UIColor clearColor];
                () = msg![env; label setBackgroundColor:clear_color];
                () = msg![env; main_view addSubview:label];
            }
